...
env = setup_environment(hyper_params["env"], hyper_params["extra_info"])
...
ac.save_network(f"Agents/{hyper_params['env']}/a2c_{hyper_params['env']}_{hyper_params['extra_info']}.pt")

#Save a video of the agent
env.reset()
make_video(env, ac, 30, 30, f"Agents/{hyper_params['env']}/Videos", 1000, f"a2c_video_{hyper_params['env']}_{hyper_params['extra_info']}.mp4")
