...
def msg_rew(env, prev_obs, action, current_obs):
    msg = current_obs[env._observation_keys.index("message")]
...
def record_video(env, agent, video_filepath, pygame_frame_rate, video_frame_rate, max_timesteps):
    ...
    """
    frame_width = env.observation_space["pixel_crop"].shape[1]
    frame_height = env.observation_space["pixel_crop"].shape[0]
    ...
    while not done and steps < max_timesteps:
        action = agent.act(normalize_glyphs(obs))
